...
5. **写真: KICKS記事のヒーローは「その靴そのものの写真」を最優先で取りに行く**（2026-09-12クリス指示「610ジャーナル。KICKSの記事の時はその靴の写真にして欲しいかな！」。それまでは新作バッシュのCC写真がほぼ存在しないため、靴の記事なのにヒーローが選手の顔写真や屋外コートの汎用写真になっていた）。**媒体（Sneaker News / Hypebeast / Nice Kicks 等）の記事画像・商品画像は従来通り使わない**（og:imageも不可。2026-09-12実測でどの媒体も画像を自社CDNにリホストしており、公式画像への直リンクは存在しない）。**例外として、ブランドが自社の商品ページで配布している画像は、ブランドへの帰属をキャプションに明記することを条件に使う。** 次の順で試し、取れた時点で止める:
   ...
   **①ブランド公式の商品画像（Nike / Jordan / adidas）**
   `python3 journal_auto/pick_product_photo.py "<モデル名 英語>" --sku <品番> --out site/assets/journal-NNN-hero.jpg`
   ...
   - **New Balance / Puma は未対応**なので①を飛ばして②へ（2026-09-12実測: newbalance.com 403）
